  ...
  <view>
    <u-popup :show="show" round="16rpx" @close="close" closeable>
      <view class="main">
        <view class="title">
          <view
            :class="['title-item', activeTitle == 1 ? 'active-title' : '']"
            @click="changeTitle(1)"
          >
            月份选择
          </view>
          <view
            :class="['title-item', activeTitle == 2 ? 'active-title' : '']"
            @click="changeTitle(2)"
          >
            自定义
          </view>
...
export default {
  ...
  methods: {
    ...
    pickerConfirm() {
      let text =
        this.customizepStartTime.start.replace(/-/g, ".") +
        "-" +
        this.customizepEndTime.end.replace(/-/g, ".");
      let obj = {
        ...this.customizepEndTime,
        ...this.customizepStartTime,
        text: text,
